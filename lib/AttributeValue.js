 ...
 */
function wrap1(val, _opts, key) {
  var opts = options.mkOpts(_opts);

  var type = getType(val, opts, key);
  if (typeof opts.reconcileFn === 'function') {
    val = opts.reconcileFn(val, opts, key, type);
    type = getType(val, opts, key);  // calling reconcile may change the type
  }
