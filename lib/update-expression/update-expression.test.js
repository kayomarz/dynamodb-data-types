...
test('yoyoma updateExpr remove', () => {
  const props = updateExpr()
        ...
        .expr();
  expect(props).toEqual({
    UpdateExpression: 'REMOVE name, game, age, tel, history',
  });
});
...
test('updateExpr avoid duplicate ExpressionAttributeNames', () => {
...
test('updateExpr allows deep equality to avoid duplicate ExpressionAttributeNames', () => {
  const expr0 = updateExpr()
        .set({ w: [1, 2, 3] })
        .set({ x: [1, 2, 3] })
        .set({ y: [1, 2, 3] })
